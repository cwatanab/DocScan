...
### キャッシュのクリアと再起動
オフライン環境下でアセットの読み込みトラブルが発生した際の復旧手段として、画面の左下にキャッシュのクリアと再起動を行うボタンを配置しました。
このボタンは、ブラウザ内のすべての Cache Storage の削除、Service Worker の登録解除、および強制的なリロードを一括して処理します。
...
コントロールバーは、キャッシュクリアボタン、シャッターボタン、ライブラリ選択ボタンを同一高さの丸ボタンで統一し、対称なレイアウトにしています。閉じるボタンは右上に配置したフローティングボタンです。
