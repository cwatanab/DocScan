...
- **PDFサイズが 1/20 に**：日本語のコピペ・検索・選択機能を100%維持したまま、A4一枚あたりのファイルサイズを約100KB前後へと極小化させました。
...
- ノッチ部分を美しく隠す磨りガラス風ヘッダーバッジ統合UIが特徴です。
- OpenCV.jsで枠線をリアルタイム検出し、画面上にグリーンのポリゴンを描画します。
- シャッターボタン押下時に、直近 8フレームのキャッシュバッファから最も手ブレの少ない1枚を自動抽出し、編集画面へ受け渡します。
