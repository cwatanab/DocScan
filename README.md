...
### サーチャブルPDFの容量最適化
ローカル配置した IPAexGothic（ipaexg.ttf）をサブセット埋め込みし、透明な文字レイヤーとして画像の上に重ね合わせます。
フォント読み込みに失敗した場合のみ Helvetica へフォールバックします。
画像は長辺 1600px・JPEG 95% 程度に再圧縮し、1ページあたりのファイルサイズを抑えています。
